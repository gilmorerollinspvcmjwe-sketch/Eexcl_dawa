...
- 修改 `hitPart` 函数：
  - 如果是 switch_track 模式，检查击中的敌人优先级是否等于 currentPriorityTarget
  - 如果不等于，返回 null（不算击中）
